...
if (isset($id)) {
    $db = getDB();
    $stmt = $db->prepare("SELECT PointsHistory.id,user_id,points_change,reason,PointsHistory.created, Users.username FROM PointsHistory JOIN Users on PointsHistory.user_id = Users.id where PointsHistory.id = :id");
...
<?php if (isset($result) && !empty($result)): ?>
    <div class="card">
		...
		<div>Date Created: <?php safer_echo($result["created"]); ?></div>
